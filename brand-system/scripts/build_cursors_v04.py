#!/usr/bin/env python3
"""Build Crow Talon v0.4: smooth high-detail Windows cursors.

The approved RGBA talon master is the exact visual source for Normal and the
...
for its proven CUR/ANI parsers and Windows loader; no v0.3 artifact is written.
"""
...
import build_cursors as v03


REPO_ROOT = Path(__file__).resolve().parents[1]
CURSOR_ROOT = REPO_ROOT / "cursors"
V04_ROOT = CURSOR_ROOT / "v0.4"
SOURCE_ROOT = V04_ROOT / "src"
WINDOWS_DIR = V04_ROOT / "windows"
PREVIEW_DIR = V04_ROOT / "previews"
MASTER_PATH = CURSOR_ROOT / "masters" / "crow-talon-master-v0.4.png"
CHROMA_PATH = CURSOR_ROOT / "masters" / "crow-talon-master-v0.4-chroma.png"
...
PACKAGE_CONTENT_HASHES_PATH = V04_ROOT / "package-files.sha256"
README_PATH = V04_ROOT / "README.md"
MANIFEST_PATH = V04_ROOT / "manifest.json"
INSTALL_PATH = V04_ROOT / "install.ps1"
UNINSTALL_PATH = V04_ROOT / "uninstall.ps1"
INF_PATH = V04_ROOT / "Crow-Talon.inf"
LICENSE_PATH = V04_ROOT / "LICENSE.md"
PREVIEW_PATH = PREVIEW_DIR / "crow-talon-v0.4-preview.png"
...
MASTER_PROFILES = {
    32: {"offset": (1, 1), "width": 29, "brightness": 1.34, "contrast": 1.14},
    48: {"offset": (2, 2), "width": 43, "brightness": 1.22, "contrast": 1.10},
    64: {"offset": (2, 3), "width": 58, "brightness": 1.12, "contrast": 1.07},
    96: {"offset": (3, 4), "width": 87, "brightness": 1.06, "contrast": 1.04},
}
NORMAL_HOTSPOTS = {32: (3, 1), 48: (5, 2), 64: (4, 4), 96: (5, 6)}
...
MASTER_HASH = "4B11093BC111889D1D4C719ADDFD8849669A70A7983E1EDEAE32BA21185458AC"
CHROMA_HASH = "682B2C3F7EC9625F1812E0DF4A086EC4DE629663B0DEA5B5C733479301316B66"
...
def load_master() -> tuple[Image.Image, tuple[int, int, int, int]]:
    ...
    if _MASTER is None:
        ...
        bbox = alpha.getbbox()
        if bbox is None:
            raise ValueError("Approved v0.4 RGBA master is empty")
        _MASTER_BBOX = bbox
...
def render_master_work(size: int) -> SmoothCanvas:
    ...
    # Reinforce major approved servos only at small native sizes. Positions are
    # mapped from the exact RGBA master; anatomy and footprint never change.
    if size <= 48:
        reinforcements = (
            ((959, 439), 0.62),
            ((502, 421), 0.42),
        ...
        # Servo highlights may never float outside the approved talon. Masking
        # them to the master's native alpha preserves the exact four-digit
        # silhouette and prevents detached cyan pixels at 32/48.
...
def draw_link_badge(canvas: SmoothCanvas) -> None:
    ...
    canvas.polygon(
        ((20.9, 23.0), (24.0, 19.9), (28.8, 22.0), (29.5, 26.4), (26.2, 29.7), (21.5, 27.8)),
        INK,
        outline=CYAN,
        width=0.45,
    )
    first = cubic_points((21.9, 24.8), (21.3, 21.4), (25.7, 21.2), (25.9, 23.5), 18)
    second = cubic_points((25.1, 25.2), (25.2, 28.5), (29.4, 28.3), (29.1, 24.1), 18)
    metal_curve(canvas, first, 1.15)
    metal_curve(canvas, second, 1.15)
    canvas.line(((24.3, 24.7), (26.8, 23.4)), FROST, 0.55)
...
def render_role(role: str, size: int, frame: int = 0) -> Image.Image:
    ...
    return finish_cursor(canvas, size, spec.hotspot(size))
...
def build_ani_bytes(spec: CursorSpec) -> bytes:
    ...
    info_list = b"INFO" + riff_chunk(
        b"INAM",
        f"Crow Talon v0.4 {spec.role}\0".encode("ascii"),
    )
    body = (
        b"ACON"
...
def validate_normal_anatomy(size: int, image: Image.Image) -> dict[str, object]:
    alpha = image.getchannel("A")
    zones = {
        "hallux": (0.02, 0.00, 0.34, 0.36),
        "digit_II": (0.00, 0.35, 0.37, 0.94),
        "digit_III": (0.28, 0.55, 0.70, 0.98),
        "digit_IV": (0.60, 0.34, 0.98, 0.94),
    }
    ...
    if min(pairs) < size * 0.10:
        raise ValueError(f"normal {size}px: talon tips collapse together")
    joint_box = (
        round(size * 0.34),
        round(size * 0.24),
        round(size * 0.78),
...
def write_normal_proof() -> None:
    ...
    draw.text(
        (24, 18),
        "CROW TALON v0.4  //  EXACT RGBA MASTER  //  NORMAL NATIVE GATE",
        font=title_font,
        fill=FROST[:3],
    )
    draw.text(
        (24, 50),
        "COOL-METAL OPTICAL LIFT + SUBPIXEL CYAN RIM; FOOTPRINT UNCHANGED",
        font=small_font,
        fill=CYAN[:3],
    )
...
def write_packaged_preview(
    ...
    draw.text(
        (20, 16),
        "CROW TALON v0.4 // PACKAGED CUR ENTRIES // NATIVE SIZE MATRIX",
        font=title_font,
        fill=FROST[:3],
    )
    draw.text(
...
def write_packaged_edge_proof(
    ...
    draw.text(
        (20, 14),
        "CROW TALON v0.4 // PACKAGED CUR ANTIALIAS EDGE AUDIT",
        font=title_font,
        fill=FROST[:3],
    )
    draw.text(
...
def write_packaged_animation_proof() -> None:
    ...
    draw.text(
        (20, 14),
        "CROW TALON v0.4 // PACKAGED ANI CONTACT SHEET // 12 MEANINGFUL FRAMES",
        font=title_font,
        fill=FROST[:3],
    )
    draw.text(
...
def immutable_snapshot() -> dict[str, str]:
    paths = [
        ...
        if path.is_file() and V04_ROOT not in path.parents
    ]
    paths.extend(
        (
    ...
    expected = {
        str((DOWNLOAD_DIR / "Crow-Talon-Windows-v0.3.0.zip").resolve()): V03_ZIP_HASH,
        str((DOWNLOAD_DIR / "Crow-Talon-Windows-v0.3.0.sha256").resolve()): V03_SIDECAR_HASH,
        str(MASTER_PATH.resolve()): MASTER_HASH,
        str(CHROMA_PATH.resolve()): CHROMA_HASH,
    }
...
def write_text_outputs(report: dict[str, object]) -> None:
    install = (CURSOR_ROOT / "install.ps1").read_text(encoding="utf-8")
    install = (
        install.replace("Crow Talon v0.3", "Crow Talon v0.4")
        .replace("$schemeName = 'Crow Talon'", "$schemeName = 'Crow Talon v0.4'")
        .replace("$packageVersion = '0.3.0'", "$packageVersion = '0.4.0'")
        .replace(
            "'Crow\\Cursors\\Crow-Talon'",
            "'Crow\\Cursors\\Crow-Talon-v0.4'",
        )
    )
    INSTALL_PATH.write_text(install, encoding="utf-8", newline="\n")

    uninstall = (CURSOR_ROOT / "uninstall.ps1").read_text(encoding="utf-8")
    uninstall = (
        uninstall.replace("Crow Talon v0.3", "Crow Talon v0.4")
        .replace("$schemeName = 'Crow Talon'", "$schemeName = 'Crow Talon v0.4'")
        .replace(
            "'Crow\\Cursors\\Crow-Talon'",
            "'Crow\\Cursors\\Crow-Talon-v0.4'",
        ...
        .replace(
            ...
            "'Crow Talon v0.4 has been removed",
        )
    )
    UNINSTALL_PATH.write_text(uninstall, encoding="utf-8", newline="\n")

    inf = (CURSOR_ROOT / "Crow-Talon.inf").read_text(encoding="utf-8")
    inf = (
        inf.replace("Crow Talon v0.3", "Crow Talon v0.4")
        .replace("Cursors\\Crow-Talon", "Cursors\\Crow-Talon-v0.4")
        .replace('"Crow Talon"', '"Crow Talon v0.4"')
    )
...
Chroma reference SHA-256: `{CHROMA_HASH}`
...
"""
    ...
    manifest_files = {
        path.relative_to(V04_ROOT).as_posix(): sha256(path)
        for root in (SOURCE_ROOT, WINDOWS_DIR, PREVIEW_DIR)
        for path in sorted(root.rglob("*"))
        if path.is_file()
    }
    manifest = {
        "name": "Crow Talon",
        "scheme": "Crow Talon v0.4",
        "version": PACKAGE_VERSION,
        "palette": ["black", "gunmetal", "cyan", "blue", "violet"],
        "master": {
            "file": MASTER_PATH.relative_to(REPO_ROOT).as_posix(),
            "sha256": MASTER_HASH,
            "chroma_sha256": CHROMA_HASH,
        },
        "sizes": list(SIZES),
        "roles": [
            {
...
def validate_archive(expected: dict[str, Path]) -> dict[str, object]:
    ...
    with zipfile.ZipFile(PACKAGE_PATH) as archive:
        ...
        with tempfile.TemporaryDirectory(prefix="crow-talon-v04-") as temporary:
...
def build() -> None:
    ...
    if before != after:
        ...
        raise ValueError(f"Immutable v0.3 guard changed: {changed}")
    print(
        json.dumps(
            {
